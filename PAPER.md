...
In late 2024, large language models crossed a threshold in code generation that was qualitatively different from what came before. Prior models could autocomplete functions and generate plausible-looking snippets. The new generation could sustain coherent implementations across thousands of lines, maintain internal consistency across module boundaries, and reason about subtle correctness properties like memory safety and concurrent access.
...
**Fault injection.** The `buggify` module, modeled on FoundationDB's BUGGIFY macro, defines 31 injectable faults across five categories: network (packet drop, corruption, reordering, delay), timer (clock drift, skips, jumps), process (crash, pause, OOM), disk (write failure, corruption, fsync failure), and replication (gossip drop, split brain). Three preset profiles -- calm (0.1x), moderate (1x), and chaos (3x) -- control overall aggression.
...
The CI pipeline runs all four layers in a single job on every push and every pull request. Every PR must pass all layers. There are no `#[ignore]` annotations, no `--skip` flags, no optional test suites. A failure in any layer blocks the merge.
...
Future work includes expanding Tcl suite coverage (bitmaps and blocking operations are the next frontier), adding persistence beyond the experimental S3 streaming layer, and exploring cluster-mode sharding across multiple machines. On the verification side, the open question is whether this methodology scales: as AI capabilities improve and the generated code grows more complex, do four verification layers remain sufficient, or does the harness itself need to evolve?
